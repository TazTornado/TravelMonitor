...
int InputParser(const char *filename, HashTable *hashtable, BloomFilterSet *bloomset, SkipListSet *vaccinated, SkipListSet *not_vaccinated){
/*
...
*/
	...
	while(getline(&recordline, &line_size, inputfile) > 0){   // read until end of file
		...
		return_value = HashInsert(hashtable, rec);
		if(return_value == 1){
			// printf("Successfully added citizen %d %s %s.\n", rec->citizenID, rec->first_name, rec->last_name);
			int retval = 0;
			...
			free(rec);  // failure of insertion means the struct isn't useful
